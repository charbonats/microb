...
| -   | `extras`                  | no       | install additional extra dependency group. Each extra must be an optional dependency group defined in the pyproject.toml                                                          | -       | `string[]`              |
| - | `requirements` | no | Path to a requirements.txt file used to install project dependencies. When requirements is specified, extras cannot be used, and dependencies listed in pyproject.toml are ignored. Use requirements when project dependencies are locked using a third-party tool like pip-tools or poetry and can be exported as a requirements.txt file. | - | `string` |
